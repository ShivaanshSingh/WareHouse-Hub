...
export async function generateBrochure(warehouse) {
  ...
  const loadImageAsBase64 = (url) => {
    return new Promise((resolve) => {
      ...
      // Use our own API proxy to avoid CORS issues with Firebase Storage
      ...
      fetch(proxyUrl)
        ...
        .catch((err) => {
          console.error('Image load failed for brochure:', err);
          resolve(null);
        });
    });
  };
  ...
  pdf.text('WarehouseHub', margin, 18);
  ...
  pdf.setFontSize(7);
  pdf.setTextColor(...white);
  pdf.text('WarehouseHub', margin, 8);
  pdf.setFont('helvetica', 'normal');
  pdf.setFontSize(6.5);
  pdf.setTextColor(180, 185, 200);
  ...
  pdf.setTextColor(...textMid);
  pdf.text(
    'Generated by WarehouseHub  |  warehousehub.in  |  All information is subject to verification.',
  ...
  pdf.setTextColor(...textMid);
  pdf.text(
    'Generated by WarehouseHub  |  warehousehub.in  |  Page 1 of 2',
    pageW / 2, footerY + 5, { align: 'center' }
  );
