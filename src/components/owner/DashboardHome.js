...
export default function DashboardHome({ setActiveTab }) {
  ...
  useEffect(() => {
    const fetchMyWarehouses = async () => {
      ...
      try {
        setLoading(true);
        const q = query(collection(db, 'warehouse_details'), where('ownerId', '==', user.uid));
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          fetchedWarehouses.push({ id: doc.id, ...data });
          calcTotalArea += Number(data.totalArea || 0);
          if (data.status === 'Live' || data.status === 'Approved') calcActive += 1;
        });
