...
export default function AddWarehouse({ setActiveTab }) {
  ...
  const [photos, setPhotos] = useState({
    frontView: null, insideView: null, dockArea: null, rateCard: null,
  });
      ...
      <div className="max-w-5xl mx-auto px-6 sm:px-10 pt-10 relative z-10">
        ...
        <div className="mb-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div>
              <h1 className="text-3xl font-bold text-slate-800 tracking-tight">List Your Warehouse</h1>
                  ...
                  <div className="p-8 sm:p-10 flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="flex flex-col gap-4">
                        <SelectField label="Type of Construction" id="typeOfConstruction" options={CONSTRUCTION_TYPES} placeholder="Select type (optional)" value={warehouseDetails.typeOfConstruction} onChange={v => {
                        ...
                        {warehouseDetails.typeOfConstruction === 'Other' && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                            <Field label="Specify Other Type" id="customTypeOfConstruction" placeholder="e.g. Mixed Construction" value={warehouseDetails.customTypeOfConstruction || ''} onChange={v => handleWarehouseChange('customTypeOfConstruction', v)} mandatory errors={errors} />
                          </motion.div>
                        )}
                      </div>
                  ...
                  <div className="p-8 sm:p-10 flex-1 space-y-10">
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Field label="Handling Rate — Optional (₹)" id="handlingRate" type="number" placeholder="Leave blank if not applicable" value={pricingDetails.handlingRate} onChange={v => handlePricingChange('handlingRate', v)} errors={errors} />
              ...
              {step < totalSteps ? (
                ...
                <button onClick={handleSubmit} disabled={submitting} className="px-8 py-3.5 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-[0_8px_20px_rgba(249,115,22,0.3)] hover:shadow-[0_12px_25px_rgba(249,115,22,0.4)] hover:-translate-y-0.5 transition-all disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0 w-64 relative overflow-hidden">
                  ...
                  <div className="relative z-10 flex items-center gap-2">
                    {submitting ? <><Loader2 className="w-4 h-4 animate-spin" /> {uploadProgress < 100 ? `Uploading... ${uploadProgress}%` : 'Saving...'}</> : <><CheckCircle className="w-4 h-4 drop-shadow-sm" /> Publish Listing</>}
                  </div>
                </button>
              )}
            </div>
